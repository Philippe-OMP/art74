...
if True:
    ...
    Vels=-40
    x1=wvl-Vels
    x2=wvl-Vels*np.sqrt(1.-beta)
    tau1=(np.sqrt(np.pi)/(beta*Vels**2))*wvl*Dopp*(erf(x2/Dopp)-erf(x1/Dopp))  
    tau2=(Dopp**2/(beta*Vels**2))*(np.exp(-x2**2/Dopp**2)-np.exp(-x1**2/Dopp**2))  
    Tau=tau1+tau2 
