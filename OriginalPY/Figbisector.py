...
for beta in [0.1,0.5,1,1.5,2]:
    ...
    for i,w in enumerate(wvl):
        ...
        #v(z)=alfa* sqrt(beta z)
        # x1=w 
        # x2=w-Vels[Amas]*np.sqrt(beta)   
        # tau1=-(2./(beta*Vels[Amas]**2))*(np.sqrt(np.pi)/2.)*w*Dopp*(erf(x2/Dopp)-erf(x1/Dopp))    
        # tau2=-(2./(beta*Vels[Amas]**2))*(Dopp**2/2.)*(np.exp(-x2**2/Dopp**2)-np.exp(-x1**2/Dopp**2))                
        # Tau=(tau1+tau2)

        #v(z)= alfa beta z
        x1=w
        x2=w-Vels[Amas]*beta
        tau1=-np.sqrt(np.pi)*Dopp/(2.*beta*Vels[Amas])*(erf(x2/Dopp)-erf(x1/Dopp))
        Tau=tau1    
...
plt.legend()  
plt.savefig('Bisectors_accel.png') 
plt.show()
